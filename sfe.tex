...
the nullspace of the Jacobian is more than one-dimensional and
we can choose the direction on it
which increases (or decreases) one particular independent variable most
rapidly. This is done by projecting a special vector onto the nullspace:
...
For example, if we want to increase a flutter speed by changing some
...
Regardless of which it is, there is a simple technique that is
cheap enough that it should be done at each step in tracking curves.

\subsection{Detecting Switching}
The technique for detecting mode switching and the much rarer phenomenon
bifurcation is the same: monitoring the sign of the determinant of the
