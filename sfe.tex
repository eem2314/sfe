...
\end{itemize}
In contrast, treating the flutter equations as a system of nonlinear
equations and tracing the evolution of each mode in a continuous fashion
with numerical continuation avoids these drawbacks:
\begin{itemize}
	\item {\bf Unified Parameter Treatment:} All parameters---including
		generalized coordinates
	    and user-defined parameters---are treated equally. Consequently,
		 every analysis is inherently a parameter variation study.
...
$\Vector{t}$ & & tangent m-vector (\Eqn{eqn:Jt}) \\
...
$\Vector{x}$ & & m independent variables \\
...
\Vector{f}(\Vector{x}) = \Vector{0} & \in \mathbb{R}^{n} \nonumber \\
\Vector{x} & \in \mathbb{R}^{m}
\end{align}
where the number of equations $n$
is fewer than the number of independent variables $m$.
Because there are fewer equations than unknowns, there are an infinite
number of solutions.
Depending on the difference $m-n$, the solution space consists of
curves ($m-n = 1$), surfaces ($m-n = 2$), volumes ($m-n = 3$), and so on
\footnote{throughout this book, only curves are considered; with the
exception of \Sectref{sect:optim}, $m = n+1$}.
...
$\Matrix{J}$, an $(n,m)$ matrix of derivatives of $\Vector{f}$ with
respect to $\Vector{x}$:
...
and because it produces the tangent vector which is used to
predict the next solution in the tracking process, errors may cause
poor predictions or even mode switching.\index{mode switching}
...
\subsection{The Predictor}\label{sect:predictor}
...
\subsection{The Corrector}\label{sect:corrector}
...
QR factorization of the transpose of $\Matrix{J}$:
\begin{equation}
\label{eqn:qr}
...
\end{equation}
where $\Matrix{Q}$ is an $(m,m)$ orthogonal matrix, $\Matrix{R}$ is
an $(m,n)$ upper-triangular matrix, and $\Matrix{R}_1$
is an $(n,n)$ upper-triangular matrix. If $m = n+1$, $\Vector{Q}_2$ is a
vector, in fact it is the null vector of the Jacobian
$\left(\Matrix{J} \Vector{Q}_2 = \Vector{0}\right)$,
useful for computing the tangent.
...
Homotopy proceeds by creating a function of $n$ equations in $n+1$
unknowns
$\Vector{g}(\Vector{y})$
from 
...
\hat{p} & = p + \Step \frac{dp}{d\tau}
	= p + \Step\sum_{j=1}^m \frac{\partial p}{\partial x_j}\frac{dx_j}{d\tau} \nonumber \\
...
The idea is straightforward: with more independent variables ($m > n+1$)
the nullspace of the Jacobian (the tangent space) is more than
one-dimensional and we can choose a direction in it that take us
toward a goal. The goal is specified by a vector $\Vector{w}$ which
is projected onto the tangent space:
\begin{equation} \label{eqn:project}
\bar{\Vector{t}} = \Matrix{Q}_2\Matrix{Q}_2^T\,\Vector{w} \quad \in \mathbb{R}^m
\end{equation}
giving a vector $\bar{\Vector{t}}$ which is closest to $\Vector{w}$, the
direction of the goal.
This yields a constrained steepest-ascent (descent) path through the
infinity of solutions make possible by the additional variables.
\index{optimization}\index{numerical continuation!optimization}

where $\Vector{w}$ points in the direction we want the curve to go. Projecting
it onto the nullspace gives a vector pointing in the direction of maximal
increase is all zeros but plus or minus 1 in the spot for
the variable to be optimized.
The tangent is just $\bar{\Vector{t}}$, normalized with
$\Vector{t} = \bar{\Vector{t}}/\|\bar{\Vector{t}}\|_2$


For example, continuation-optimization is used in
\Sectref{ex:goland-contour} to obtain starting points for tracing
contours of dynamic pressure as a function of aerodynamic center and
center of gravity.
if we want to increase a flutter speed by changing some
parameter values we could include all the relevant parameters in the
solution and choose the vector $\Vector{w}$ to be all zeros except
a $1$ in the element corresponding to velocity. Computing the tangent
with this $\Vector{w}$ then gives a path towards higher velocities
with respect to all the parameters that were included.

\begin{figure}[ht]
		\includegraphics[height=8cm,width=15cm]{asy/optim3.pdf}
	\centering
	\caption{Tangent by projecting $\Vector{w}$ onto the nullspace with $m=n+2$ }
	\label{fig:optim}
\end{figure}
...
\begin{itemize}
	\item nonconvergence of the corrector: retry the step with a smaller
		stepsize; if this does not cure the problem the Jacobian should
		be checked by finite differencing.
	\item large angle between consecutive tangents. This may indicate
		a slope discontinuity in the curve, mode switching, a bifurcation,
		...
		\index{mode switching!detecting}
		...
		\begin{enumerate}
			\item reducing the stepsize reduced the angle substantially so there
				is not a slope discontinuity or mode switching,
			\item the angle has not reduced substantially so there appears to
				be a slope discontinuity between two of the steps taken
				while monitoring,
		...
		\end{enumerate}
	\item a change in the sign of the determinant of the Jacobian matrix
			augmented with the tangent vector (\Sectref{sect:bifurcation}),
			indicating mode switching or (rarely) bifurcation.
	\item a singular Jacobian indicates a bifurcation; very rare and
		a bifurcation is more likely to be discovered with a change in
		the sign of the determinant.
\end{itemize}
Mode switching can be very serious if undetected. Fortunately both checking
the angle between subsequent tangents and the sign of the determinants
are inexpensive and easily implemented \Sectref{sect:bifurcation}.

\section{Mode Switching and Bifurcation}\label{sect:bifurcation}
\index{bifurcation}
When tracing a curve with numerical continuation it is possible
for the tracing to jump to another curve, especially when
2 curves come close to one another and the algorithm takes too large
a step for the local curvature.
This so-called \emph{mode switching} can also be an indication that a
bifurcation has been passed, where two curves become equal within
the limits of floating-point computation.
Regardless of which it is, there are two simple techniques that are
cheap enough that they should be done at each step in tracking curves.

\subsection{Detecting Switching}
The first test is to monitor the angle between tangents at subsequent
solution points: $cos^{-1}\left(\Vector{t}_{j-1}^T\Vector{t}_j\right)$.
If the angle is greater than, say 40 degrees, repeat the step with a
smaller stepsize.

The second technique for detecting mode switching and the much rarer phenomenon
bifurcation is to monitor the sign of the determinant of the
Jacobian matrix augmented with the tangent vector,
\index{mode switching!detecting} \index{bifurcation!detecting}
	...
	\det \left[
		\Matrix{J}^T \;\; \Vector{t}
	\right]$.
Assuming the predictor-corrector scheme of \Sectref{sect:predictor}
and \Sectref{sect:corrector}, with convergence there is a QR factorization
of the transposed Jacobian which can be used to compute the determinant
efficiently.
...
%% 	\right]
%% \end{equation*}
%% a change in sign indicates the orientation has changed and either
%% the tracking algorithm has jumped to a different curve (mode switching)
%% or a bifurcation has been passed (\cite{allgower1990numerical}, p. 81).
\par
Evaluating the determinant is simplified by recognizing that
%% the tangent came from the QR factorization, that
the determinant of an orthogonal matrix is $\pm 1$,
\footnote{the sign of the determinant of $\Matrix{Q}$ is determined by the
number of Householder reflections in the factorization, as pointed out in
\Sectref{sect:langou}.}
and the determinant of a triangular matrix
...
Premultiplying the augmented Jacobian by $\Matrix{Q}^T$
\begin{align*}
	\Matrix{Q}^T \left[ \Matrix{J}^T \;\; \Vector{t} \right] =
		\left[ \Matrix{R} \;\; \Matrix{Q}^T \Vector{t} \right] =
		\tilde{\Matrix{R}}
\end{align*}
Assuming we are tracing curves and the (n,m) Jacobian has one more
column than rows, the transposed Jacobian has a QR factorization
with an (m,n) R with the form
\begin{align*}
	\Vector{R}: \left[ \begin{array}{ccc}
		X & X & X \\
		0 & X & X \\
		0 & 0 & X \\
		0 & 0 & 0
		\end{array} \right]
\end{align*}
and appending the tangent to the last column results in a square
(m,m) upper-triangular matrix $\tilde{\Matrix{R}}$
\begin{align*}
	\tilde{\Matrix{R}} =\left[\Vector{R} \;\; \Matrix{Q}^T\Vector{t} \right]:
		...
		\end{array} \right]
\end{align*}
so the determinant of the augmented Jacobian is
\begin{align*}
	\det \left[ \Matrix{J}^T \;\; \Vector{t} \right] =
		\det\left( \Matrix{Q}\tilde{\Matrix{R}} \right) =
		\det\left( \Matrix{Q} \right) \det\left(\tilde{\Matrix{R}} \right) =
		(-1)^k \prod_{i=1}^m \tilde{R_{ii}}
\end{align*}
where $k$ is the number of Householder reflections.
\footnote{In LAPACK routine DGEQRF the number of reflections is the
number of nonzero elements in the TAU array \Sectref{sect:langou}}.

Because determinants are usually very large or very small, they must
be treated as 2 numbers: a coefficient and an exponent. Only the sign
of the coefficent is necessary for detecting mode switching or bifurcation.

\subsection{Locating a Bifurcation}
...
matrix approaches singularity. A final approximation can be obtained by
interpolation with
	$\mu(\Vector{x}) = \det\left[\Matrix{J}(\Vector{x})^T \;\; \Vector{t}\right]$:
\begin{equation}
...
\Matrix{J} = \Matrix{U}\Matrix{\Sigma}\Matrix{V}^T, \hspace{0.2in} 
	\Matrix{U} \in \mathbb{R}^{n \times n}, \;\;\;
	\Matrix{V} \in \mathbb{R}^{n+1 \times n+1}		\nonumber
\end{equation}
...
$\Matrix{\Sigma} = \Diag (\sigma_1, \dots \sigma_m) \in \mathbb{R}^{n \times n+1}$
...
At a regular point on the curve the last column of $\Matrix{V}$, $\Vector{v}_{n+1}$,
is a Jacobian null vector,
and (within a sign) is the tangent vector.
At a bifurcation point the last column of $\Matrix{U}$ ($\Vector{u}_n$),
and the last two columns of $\Matrix{V}$ are left and right null vectors, respectively:
\begin{align}
\Vector{u}_n^T\Matrix{J} &= \Vector{0} \nonumber \\
\Matrix{J}\Vector{v}_n &= \Matrix{J} \Vector{v}_{n+1} = \Vector{0},
\end{align}
and there are two tangents,
combinations of the right null vectors:
\begin{equation}
\label{eqn:tanb}
\Vector{t}_k = \alpha_k \Vector{v}_n + \beta_k \Vector{v}_{n+1}, \hspace{0.2in} k=1,2.
\end{equation}
...
\begin{equation}
\Vector{u}_n^T\Vector{f}''\dot{\Vector{x}}\dot{\Vector{x}} =
	\Vector{u}_n^T\Vector{f}''\Vector{t}\Vector{t} = 0.
\end{equation}
Substituting \Eqn{eqn:tanb}
\begin{align}
\Vector{u}_n^T\Vector{f}'' (\alpha_k\Vector{v}_n &+ \beta_k\Vector{v}_{n+1}) (\alpha_k\Vector{v}_n + \beta_k\Vector{v}_{n+1}) = \nonumber \\
 &     \Vector{u}_n^T\Vector{f}''\Vector{v}_n\Vector{v}_n \alpha_k^2 + 2\Vector{u}_n^T\Vector{f}''\Vector{v}_n\Vector{v}_{n+1}\alpha_k\beta_k + \Vector{u}_n^T\Vector{f}''\Vector{v}_{n+1}\Vector{v}_{n+1}\beta_k^2 \nonumber \\
...
\begin{align*}
\Vector{t}_k^T\Vector{t}_k &= \alpha_k^2\Vector{v}_n^T\Vector{v}_n
	+ 2\alpha_k\beta_k\Vector{v}_n^T\Vector{v}_{n+1}
	+ \beta_k^2\Vector{v}_{n+1}^T\Vector{v}_{n+1} \nonumber \\
...
\begin{align*}
 a_{11} &= \Vector{u}_n^T\Vector{f}''\Vector{v}_n\Vector{v}_n, \nonumber \\
 a_{12} &= \Vector{u}_n^T\Vector{f}''\Vector{v}_n\Vector{v}_{n+1}, \nonumber \\
 a_{22} &= \Vector{u}_n^T\Vector{f}''\Vector{v}_{n+1}\Vector{v}_{n+1},
\end{align*}
containing
...
To this end let
\begin{equation*}
g(\xi_1,\xi_2) = \Vector{u}_n^T\Vector{f}(\Vector{x}_b
	+ \xi_1 \Vector{v}_n + \xi_2 \Vector{v}_{n+1}).
\end{equation*}
...
\begin{equation}\label{eqn:jac-omega}
	\frac{\partial \Matrix{r}}{\partial \omega} \Leftrightarrow
		\left[ 2is\Matrix{M} + i\left(\Matrix{G} + \Matrix{C}\right)
			- q\frac{b}{V}\frac{\partial \Matrix{Q}}{\partial \bar{\omega}}
...
\item[first uncoupled bending freq (cg=0)] 50 rad/sec = 7.96 Hz
...
\begin{equation}
f_i \le \epsilon \sum_{j=1}^{m} |J_{ij} x_i| = \epsilon_i
\end{equation}
This same method can be used to compute $\epsilon_a$ in \Eqn{eqn:tradcc} as
\begin{equation}
\epsilon_a = \sqrt{\sum_{i=1}^{n} \epsilon_i} = \|\{\epsilon_i\}\|_2
\end{equation}
...
\section{Determinant of Q}\label{sect:langou}
When computing the determinant of a QR factorization, the determinant
of $\Matrix{Q}$ is $\pm 1$ with the sign determined by the number
of Householder reflections performed in the factorization,
as pointed out by Julien Langou in a posting to the lapack-forum:
\begin{lstlisting}
