...
curves, $n = m+1$, and $\Vector{Q}_2$ is a vector.
...
\begin{align*}
	\Matrix{Q}^t \left[ \Matrix{J}^t \;\; \Vector{t} \right] =
		...
		\tilde{\Matrix{R}}
\end{align*}
Assuming we are tracing curves and the (m,n) Jacobian has one more
column than rows, the transposed Jacobian has a QR factorization
with an (n,m) R with the form
\begin{align*}
...
(n,n) upper-triangular matrix $\tilde{\Matrix{R}}$
\begin{align*}
