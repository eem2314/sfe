...
\end{equation}
where $\Matrix{Q}$ is an $(m,m)$ orthogonal matrix, and $\Matrix{R}_1$
is an $(n,n)$ upper-triangular matrix. Because we are only considering
curves, $m = n+1$, and $\Vector{Q}_2$ is an m-vector, in fact it is the
null vector of the Jacobian $\left(\Matrix{J} \Vector{Q}_2 = \Vector{0}\right)$,
...
\bar{\Vector{t}} = \Matrix{Q}_2\Matrix{Q}_2^t\,\Vector{w} \;\; \in \mathbb{R}^m
\end{equation}
...
%% A further simplification is possible by accounting for the fact
%% that the tangent is a null vector of the Jacobian, so
%% $\Matrix{Q}^t \Vector{t}$ will be approximately zero except for
%% the last element which will be approximately $\pm 1$ since
%% the last column of $\Matrix{Q}$ is (within a sign) the tangent.
%% An even cheaper version of the determinant is therefore
%% \begin{align*}
%% 	\det \left[ \Matrix{J}^T \;\; \Vector{t} \right] =
%% 		(-1)^k \Vector{Q}_{:m}^t\Vector{t} \prod_{i=1}^m \tilde{R_{ii}}
%% \end{align*}
