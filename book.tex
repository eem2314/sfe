...
% $\Vector{\hat{f}}_y$, $\Vector{f}_y$ & & complex residual and its real
...
% $\Omega$ & & spin rate (gyroscopic components) \\
...
This is a system of $n_e$ complex equations, homogeneous in $\Vector{\hat{y}}$,
and nonlinear in one or more parameters. In spite of this, it is termed
a \emph{linear} flutter equation if none of the matrices are
...
\end{description}
% A major drawback to these methods is that because they solve the equations
% as an eigenvalue problem, they cannot be used with controls equations
% or nonlinearities and are tedious to use for parameter studies.
% Gyroscopic terms are sometimes possible with modifications to the formulation.
% 
% Another drawback is that solutions generally use unsteady aerodynamics at
% a fixed Mach number, matching the true Mach number at only one velocity,
% so some sort of iteration or interpolation
% is necessary to produce a so-called \emph{matched Mach} solution
% \cite{demasi2024introduction,pitt1999new}.
% 
% In contrast, solving the flutter equations as a system of nonlinear equations
% avoids these drawbacks; the flutter equation used here (\Eqn{eqn:flut})
% allows control systems, gyroscopics, viscous damping, nonlinearities, unsteady
% aerodynamics which are functions of complex reduced frequency ($\bar{s}$)
% and Mach number. Any of the matrices can be quasi-linear functions of
% discrete \Gc.
...
% Another notable drawback is that these solutions typically rely on
% unsteady aerodynamics at a fixed Mach number. This means the true
% Mach number is accurately represented at only a single velocity point,
% necessitating iteration or interpolation to achieve a "matched Mach" solution.
...
% so-called pseudo-arclength methods take steps along the tangent
...
% This is perhaps the simplest numerical continuation method to explain.
...
% The tangent is always derived from the null vector of the Jacobian
...
% The distance along the curve is also known as the \emph{arclength};
% the small steps taken in the predictor then are approximations to a
% small change in arclength, hence the name \emph{pseudo-arclength}
% continuation.
% Computing the tangent will be a side-effect of the corrector process.
...
% predicted solution: a) constraining the continuation parameter,
% b) constraining corrections to be orthogonal to the tangent,
% c) minimum-norm.
\begin{figure}[ht] 	% complains if h
...
% There are many AD libraries for most modern programming languages \cite{autodiff};
% for application to flutter equations many of the features of these
% libraries are unnecessary

% The primary disadvantage of AD is the requirement of a specialized library.
% While many excellent, freely available AD libraries exist \cite{autodiff},
% the unique requirements of flutter analysis and the \Fina framework
% necessitated the creation of its own custom AD library, tailored
% to meet the specific demands of the solver.
...
\end{lstlisting}
% In \Fina this concept is realized by the datatype called
% \Code{Ad}\index{Ad@\Code{Ad} datatype}
% which stands in place of a standard floating-point variable.

% Crucially, each variable of type Ad is structured to store not just
% a single real value, but also one or more derivatives. That is, each
% \Code{Ad} variable has a primary real value plus a vector of its derivatives
% with respect to a set of designated independent parameters.

% These components are typically accessed via dedicated members. For example,
% the following code:
...
% Move this section to Custom functions? or to the Fina manual
% \subsection{Programming with AD}
% A programmer can use this AD implementation without writing
% any explicit differentiation code. The only change required from normal coding
% practice is the use of the \Code{Ad} datatype in place of a standard
% floating-point datatype, and identifying the independent variable by
% setting it's \Code{der} to 1.
% 
% The following example illustrates a program that calculates both a value
% and its derivative:
% \begin{lstlisting}
%    Ad myfunc(Ad s) {
...
%       Ad s;
%       s.value = 10.0;
%       s.der = 1.0;    // Initialize s as the independent parameter
% 
%       Ad a = myfunc(s);
% 
%       std::cout << "a = " << a.value << std::endl;
%       std::cout << "derivative of a wrt s = " << a.der << std::endl;
%    }
% \end{lstlisting}
% In this stylized example, s is designated as the independent variable.
% Compiling and running this program yields output (with calculated values for
% \Code{s=10.0}):
% \begin{lstlisting}
%     a = 33.61559
%     derivative of a wrt s = 1.78885
% \end{lstlisting}
% The derivative result is propagated through the function's entire
% computation automatically.
% 
% \section{\Fina\ Implementation}
% Automatic Differentiation is used extensively throughout \Fina, not only
% for basic parameter values but critically for evaluating parametrized
% matrices. With all parameters and matrices defined in terms of \Code{Ad}
% objects, the calculation of the Jacobian ---the heart of numerical
% continuation---becomes just a matter of extracting derivatives from the
% resulting terms of the function. For instance, this applies to the flutter
% function $\Vector{Dy}$, where $\Vector{D}$ is the dynamic matrix and
% $\Vector{y}$ is the eigenvector (defined in terms of \Code{Ad} if there
% are nonlinear generalized coordinates, or as simple complex otherwise).
% 
% \subsection{Custom Functions}
% The most important consequence of \Fina's AD implementation is that it
% makes user-written custom functions feasible. The complexity of derivative
% computation is effectively hidden from the user through the established
% mechanisms of operator and function overloading and \Cpp\ type aliases.
% 
% \subsubsection{AD-Aware Type Aliases}
% The core \Cpp\ type aliases that enable derivative propagation within
% user-written custom functions are:
% \begin{lstlisting}
% using Real = Ad;
% using Complex = std::complex<Ad>;
% using RealMatrix = matrix<Ad>;
% using ComplexMatrix = cmatrix<Ad>;
% \end{lstlisting}
% By using these aliases instead of raw \Code{double} or
% \Code{std::complex<double>}, the user
% implicitly engages the AD system for all arithmetic operations.
% 
% \subsubsection{A User-Written Example}
% A typical user-written custom function, designed to modify the stiffness matrix
% based on a user-defined parameter, looks like this:
% \begin{lstlisting}
% int stiffcn(Pset& pset, Mset& mset, ComplexMatrix& ca) {
%     Real sbfreq;
%     pset.parval("sbfreq", sbfreq);
%     
%     Complex mass;
%     mset.element("mass", 41, 41, mass);
%     
%     // Stiffness = mass * (natural frequency)^2
%     ca(41, 41) = sbfreq * sbfreq * mass;
%     
%     return 0;
% }
% \end{lstlisting}
% 
% This example illustrates three key components of the \Fina\ system:
% 
% \paragraph{Parameter Access (\Code{Pset})}
% \Code{Pset} is a \Cpp\ class containing all the standard and user-defined
% independent and dependent parameters for the solution. The call
% \Code{pset.parval("sbfreq", sbfreq)} invokes the specialized \Code{parval}
% member function. This function not only evaluates the user-defined
% parameter value but also returns the result in the \Code{Real} variable
% \Code{sbfreq}, ensuring all necessary derivatives (typically $1.0$
% if it is an independent parameter, or a calculated derivative if it is
% dependent) are correctly initialized in the \Code{Ad} structure.
% 
% \paragraph{Matrix Element Access (\Code{Mset})}
% \Code{Mset} holds all the major matrices specified for a flutter
% solution. The member function \Code{element} provides access to a
% specific matrix (identified by its role, e.g., ``mass'') and returns an
% element according to row and column indices. In the example, the diagonal
% element $(41, 41)$ of the mass matrix is accessed and returned into the
% \Code{Complex} variable \Code{mass}.
% 
% \paragraph{Derivative Propagation}
% The final assignment, \Code{ca(41, 41) = sbfreq * sbfreq *
% mass;}, is the core of the implementation. The statement uses the
% single-degree-of-freedom relation $\omega = \sqrt{k/m}$ to calculate the
% equivalent stiffness and assigns it to the diagonal of the return matrix
% \Code{ca}. Since \Code{sbfreq} and \Code{mass} are built on \Code{Ad}
% and \Code{std::complex<Ad>}, the overloaded operators ensure that the
% resulting \Code{ca(41,41)} element contains the correct real value
% \textit{and} the mathematically exact derivatives with respect to all
% independent parameters, making the final matrix directly usable for
% Jacobian assembly.
...
% Combined, \Eqna{eqn:normalization}{eqn:flut} are $2n_e+2$ real equations, so
% to compute curves (see \Sectref{sect:background})
% there needs to be $2n_e+3$ unknowns. The components of
% the eigenvector $\Vector{y}$ account for $2n_e$ unknowns; the remaining 3
% are chosen from the parameters comprising the flutter equation
% (\Sectref{sect:flut-nomenclature}).
% The eigenvector $\Vector{y}$ is not considered independent as it is
...
% In addition to choosing the independent variables it is necessary to assign
% equations (which may be constants) to the remaining parameters.
% \Fina chooses equations from \Tableref{table:params-eqns} after the user has
% specified independent and constant parameters.
...
% \footnote{these are not the only parameters that could be used for a core analysis.
% $V_e, V_c, \bar{\omega}, q, \;\text{or}\; M$ could be used instead of $V$. Likewise
% $d\;\text{or}\;g$ could be used instead of $\sigma$}:
\begin{displaymath}
...
% \footnote{Output units in the SI system are the same as equation units with the
% exception of frequency ($\omega$) which is output in Hz.}.
...
% Thus, the determination of limit cycle stability
...
\begin{itemize}
	\item a custom \Cpp function written by the user,
...
% The flutter equation used in \Fina (\Eqn{eqn:flut}) can be written
% \begin{align}\label{eqn:lti-struct}
% \Vector{Dq} &= \Vector{f} \;\; \in \mathbb{C}^{n_e} \nonumber \\
% \Matrix{D} &=
% 	\left[ s^2 \Matrix{M} + s \Omega\Matrix{G} + s \Matrix{C} +
...
% of freedom.
The control-system input $\Vector{u_i}$ is related to the
\Gc\ $\Vector{q}$ by
\begin{equation}\label{eqn:abcd-uq}
\Vector{u_i} = \Matrix{S\Phi}_i \Vector{q}
\end{equation}
...
the \Gc), and $\Matrix{S}$ is an $(n_i,n_i)$
...
Substituting \Eqn{eqn:abcd-uq} into \Eqn{eqn:abcd}
\begin{equation*}
s\Vector{v} = \Vector{Av} + \Matrix{BS\Phi}_i \Vector{q}
\end{equation*}
or
\begin{equation}\label{eqn:abcd5}
\Matrix{BS\Phi}_i \Vector{q} + \left( \Matrix{A} - s\Matrix{I} \right) \Vector{v} = 0
\end{equation}

% If the control-system output $\Vector{u_o}$ is related kinematically to
% the \Gc\ by an $(n_e, n_o)$ matrix $\Matrix{E}$
% then the external force applied to the structure due to $\Vector{u_o}$ is
Assuming the output from the control system moves one or more generalized
...
\Vector{f}_o = \Matrix{K}\Matrix{\Phi}_o\Vector{u}_o
\end{equation}
where $\Matrix{\Phi}_o$ is an ($n_e, n_o$) transformation matrix.

Substituting \Eqn{eqn:abcd} and \Eqn{eqn:abcd-uq} into \Eqn{eqn:lti-f}
\begin{equation}\label{eqn:abcd-force}
\Vector{f}_o = \Matrix{K\Phi}_o \left( \Vector{Cv} + \Matrix{D}\Vector{u}_i \right) = \Matrix{K\Phi}_o \left( \Vector{Cv} + \Matrix{DS\Phi}_i \Vector{q} \right)
\end{equation}
and since\footnote{$\hat{\Matrix{D}}$ is the dynamic matrix, not to be confused with $\Matrix{D}$
in \Eqn{eqn:abcd}} $\hat{\Matrix{D}}\hat{\Vector{y}} - \Vector{f}_o = 0$,
\Eqn{eqn:abcd5} and \Eqn{eqn:abcd-force} can be assembled into a matrix:
\begin{equation}\label{eqn:abcd-combtd}
\left[ \begin{array}{cc}
	\hat{\Matrix{D}} -\Matrix{K}\Matrix{\Phi}_o\Matrix{DS\Phi}_i
	& -\Matrix{K}\Matrix{\Phi}_o\Matrix{C}   \\
	\Matrix{BS\Phi}_i  &  \Matrix{A} - s\Matrix{I}
\end{array} \right]
\left\{ \begin{array}{c}
\Vector{q} \\
\Vector{v}
...
% When the input gain
% $(\Matrix{G})$
% is zero, the second row of equation
...
\begin{equation}
\Matrix{T} = \left[ \begin{array}{cc}
	-\Matrix{K\Phi}_o\Matrix{DS\Phi}_i & -\Matrix{K}\Matrix{\Phi}_o\Matrix{C} \\
	\Matrix{BS\Phi}_i  &  \Matrix{A} - sI
\end{array} \right]
\end{equation}
To account for the additional $n_s$ \Gc\ ($\Vector{v}$)
...
\end{array} \right\}
\end{equation}
and $n_s$ rows and columns of zeros must be added to
...
% A critical aspect of flutter analysis is conducting parameter studies
% – investigating the boundary between stability and instability across a range
% of flight parameters (e.g., velocity, altitude) and design parameters
% (e.g., stiffnesses, mass distribution, structural damping).
% In essence, flutter analysis inherently involves a series of such studies.
...
% \Fina has a set of pre-defined parameters which are sufficient for
% many flutter analyses. Additional parameters can be defined and
% applied to flutter analyses, used in the same way builtin parameters
% are used.
...
\input{params-names.tex}

% Not all parameters in \Tableref{table:gpset-eqns} appear in \Eqn{eqn:flut}, where
% the main candidates for independent parameters appear to be the Laplace
% variable $s$ and the dynamic pressure $q$.
...
The \Cmd{fem} command is used to model the
...
\begin{figure}[ht]
	...
	\caption{2 modes tracked using various formulations: vtas-g}
			\label{fig:methodsvg}
\end{figure}
% \Figrefs{fig:methodsgf,fig:methodsvg}
...
% The \Cmd{pz} command can approximate
% \footnote{unlike interpolation, which passes through a set of points,
% an approximation in general does not}
% generalized unsteady aerodynamic matrices using a rational function
...
% \Fina uses Matrix Market format mainly for visualizing matrices
...
% difference between 2 matrices (as done in the \Fina script
% \Cmd{diffmm} \Sectref{sect:utility}).
% Here are 3 \Octlab functions included in \Fina for importing,
...
% \bibliography{/home/eem2314/bib/repo/eem}
